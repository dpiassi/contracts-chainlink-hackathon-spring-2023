...
contract Order {
  ...
  modifier onlyDeliveryContract() {
    require(msg.sender == deliveryContract, "You aren't the delivery contract");
    _;
  }
  ...
  function assertLatLng(int32 _latitude, int32 _longitude) private pure returns (LatLng memory latLng) {
  ...
  function assertLatitude(int32 latitude) private pure {
    require(latitude <= MAX_LATITUDE && latitude >= MIN_LATITUDE, "Latitude must be between -90 and 90 degrees");
  }

  function assertLongitude(int32 longitude) private pure {
    require(longitude <= MAX_LONGITUDE && longitude >= MIN_LONGITUDE, "Longitude must be between -180 and 180 degrees");
  }
}
